    ...
    <div className="sidebar px-[1rem]">
      ...
      <div className="py-[3rem]">
        {sidebarLinks.map(({ title, icon }, index) => (
          <div
            className={`flex cursor-pointer rounded-[0.6rem] px-[1rem] py-[1rem] text-[#787486] transition-all duration-100 hover:bg-[#5030e514] hover:text-[#0D062D] ${
              index !== sidebarLinks.length - 1 && 'mb-2'
            }`}
          >
            <img className="mr-8 h-[2.4rem] w-[2.4rem]" src={icon} alt="" />
            <p className="text-[1.6rem] font-medium  capitalize">{title}</p>
          </div>
        ))}
      </div>
      ...
      <div className="py-[3rem]">
        <div className="mb-[2rem] flex items-center justify-between px-[1rem]">
        ...
        {myProjectsSidebarLink.map(({ color, title }) => (
          <div className=" mb-[1rem] flex h-[4rem] items-center justify-between rounded-[0.6rem] px-[1rem] text-[#787486] transition-all duration-100 hover:bg-[#5030e514] hover:text-[#0D062D]">
